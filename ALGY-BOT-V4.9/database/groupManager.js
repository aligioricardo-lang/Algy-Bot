...
class GroupManager {
  ...
  async updateGroupActivity(groupId) {
    try {
      ...
      groupData.lastUpdate = timestamp;
      groupData.eventType = 'message_activity';
      ...
      // Log da atividade
      console.log(chalk.hex('#DDA0DD').bold(`📝 [${new Date().toLocaleTimeString('pt-BR')}] Atividade atualizada`));
      console.log(chalk.hex('#DDA0DD')(`   └─ Grupo ID: ${groupId}`));
      
      return groupData;
    } catch (error) {
      console.log(chalk.red(`❌ Erro ao atualizar atividade do grupo: ${error.message}`));
      return null;
    }
  }
  ...
  async saveGroupData(sock, groupId, eventType = 'update') {
    try {
      ...
      // NOTA: A função saveGroupData é para ser chamada apenas em eventos de mudança de metadados (subject, admins, etc.)
      // Para atividade de mensagem, use updateGroupActivity para evitar rate-limit.
